...
export default function RiskPanel({
  ...
  useEffect(() => {
    ...
    // touch quicker. All bookkeeping lives OUTSIDE the setState updater —
    // StrictMode double-invokes updaters, so an impure one silently stalls.
    ...
    const id = setInterval(() => {
      ...
      for (const k of PACED_KEYS) {
        ...
        const shown = shownRef.current[k] || 0
        const cadence = CADENCE[k] ?? CADENCE.default
        if (shown < total && (!last[k] || now - last[k] >= cadence)) {
          last[k] = now
          advanced.push(k)
        }
      }
      if (advanced.length) {
        ...
        for (const k of advanced) next[k] = (next[k] || 0) + 1
