...
import {
  ...
  ZelleGlyph, InfoDot, BlueInfoDot, PrinterIcon,
} from './icons.jsx'
...
// Zelle send flow: form → review → evaluating (risk panel) → challenge/paused/success.
// Telemetry is captured live from modal-open; results come from the mock module
// (api.js) until the Phase 4 backend lands.
export default function ZelleFlow({ balance, onClose, onSent }) {
  ...
  const [adjudicator, setAdjudicator] = useState(null)
  ...
  function routeDecision(adj) {
    // Small beat so the audience can read the panel before the verdict routes.
    routeTimer.current = setTimeout(() => {
      if (adj.decision === 'allow') finalizeSuccess()
      else if (adj.decision === 'step_up') setStep('challenge')
      else setStep('paused')
    }, 1600)
  }

  function handleEvent(evt) {
    if (evt.agent === 'adjudicator') {
      setAdjudicator(evt)
      routeDecision(evt)
    } else {
      setAgentResults((prev) => ({ ...prev, [evt.agent]: evt }))
    }
  }

  function handleSend() {
    const telemetry = summarize(recorder.current, reviewShownAt.current)
    const payload = {
      transaction: {
        amount: amountNum,
        payee: payeeName,
    ...
    console.log('[NeuroSecure] Send payload →', JSON.stringify(payload, null, 2))
    setAgentResults({})
    setAdjudicator(null)
    setStep('evaluating')
  ...
  function handleChallengeContinue() {
    ...
    setStep('sending')
    cancelMock.current = evaluate(payload, (evt) => {
      if (evt.agent === 'adjudicator') finalizeSuccess()
    })
  }
      ...
      <div className="modal">
        ...
        {step === 'evaluating' && (
          <>
            ...
            <RiskPanel results={agentResults} adjudicator={adjudicator} />
          </>
        )}
        ...
        {step === 'challenge' && (
          <>
            ...
            <p className="challenge-msg">{adjudicator?.customer_message}</p>
        ...
        {step === 'paused' && (
          <>
            ...
            <p className="paused-msg">{adjudicator?.customer_message}</p>
