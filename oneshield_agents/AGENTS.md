...
UI "Send money"                                  (frontend, REAL toggle)
  └─ POST api/evaluate ──► backend/app.py
       ...
       └─ ONESHIELD_AGENT=1 ──► oneshield_agents.run_investigation(slices…)
...
- MOCK (UI toggle): zero agents, scripted tapes in `frontend/src/mock.js` —
  the demo safety net.
- REAL + `ONESHIELD_AGENT=1`: everything above (this file).
- REAL + `ONESHIELD_AGENT=0` (legacy / interactive-consent runs):
  `backend/orchestrator.py` — same six roles, but the router is the v2.2
  Sentinel loop (parallel batches) instead of LangGraph tool-calling.
