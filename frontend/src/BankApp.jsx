...
import { isMock, setMockMode } from './config.js'
...
export default function BankApp() {
  ...
  // Mock vs Real backend — runtime toggle in the rail header. Mock stays the
  // startup default; Real routes the next run to the FastAPI backend.
  const [mockMode, setMockModeState] = useState(isMock())
  ...
  // Backend fixture names differ from card keys (grandparent→coerced,
  // remote→bot); mock.js understands the card keys directly. Reads isMock()
  // (module state, synchronous) rather than the mockMode render state so a
  // toggle-then-arm in quick succession can never send a stale key.
  function scenarioApiKey(card) {
    if (!card) return 'normal'
    return !isMock() && card.backendKey ? card.backendKey : card.key
  }

  function switchMode(nextMock) {
    if (nextMock === mockMode) return
    setMockMode(nextMock)
    setMockModeState(nextMock)
    // Re-arm the current scenario on the side we just switched to, so the
    // next send behaves the same no matter when the toggle was flipped.
    const card = scenario
    const name = !card ? 'normal' : (!nextMock && card.backendKey ? card.backendKey : card.key)
    apiSetScenario(name).catch(() => {})
  }

  function pickScenario(card) {
    const next = scenario?.key === card.key ? null : card
    setScenarioCard(next)
    apiSetScenario(scenarioApiKey(next)).catch(() => {})
      ...
      <aside className="scenario-rail">
        <div className="sr-head">
          <div className="sr-head-row">
            ...
            <div className="sr-mode" role="group" aria-label="Backend mode">
              <button
                className={`sr-mode-btn${mockMode ? ' active' : ''}`}
                onClick={() => switchMode(true)}
              >
                Mock
              </button>
              <button
                className={`sr-mode-btn${!mockMode ? ' active real' : ''}`}
