...
export default function ZelleFlow({ balance, biometricConsent, displayMode = 'live', devMode = false, scenario = null, onClose, onSent }) {
  // Scripted demo runs open straight at review, prefilled — one tap to send.
  const [step, setStep] = useState(scenario ? 'review' : 'form')

  // form state
  const [recipient, setRecipient] = useState(scenario ? scenario.payee : '')
  const [chosenPayee, setChosenPayee] = useState(null) // known payee object, or null
  const [amount, setAmount] = useState(scenario ? String(scenario.amount) : '')
  const [memoOpen, setMemoOpen] = useState(false)
  const [memo, setMemo] = useState('')
  const [acknowledged, setAcknowledged] = useState(false)

  // evaluation state (v2 wire format: step / result / adjudication events;
  // v2.2 adds sentinel meta + invoke events for the Sentinel-routed run)
  const [results, setResults] = useState({})
  const [shieldSteps, setShieldSteps] = useState({})
  const [adjudication, setAdjudication] = useState(null)
  const [invocations, setInvocations] = useState({})   // shield -> reason line
  const [shieldOrder, setShieldOrder] = useState([])   // cards in appearance order
  const [sentinelModel, setSentinelModel] = useState(null)
  // A card may only appear after the sentinel line that caused it has been
  // REVEALED on screen — the audience reads the decision, then sees it act.
  const [appearAfter, setAppearAfter] = useState({})   // shield -> sentinel line count
  const sentinelLineCount = useRef(0)
  ...
  // Answer pacing on the challenge screen: both rushed and unusually slow
  // answers are red zones — an on-device signal (Shantanu, Jul 22 sync).
  const challengeShownAt = useRef(0)
  const [lastAnswerAt, setLastAnswerAt] = useState(null)
  ...
  useEffect(() => {
    ...
    if (step === 'challenge') { challengeShownAt.current = performance.now(); setLastAnswerAt(null) }
  }, [step])
  ...
  function noteShield(shield) {
    if (shield === 'sentinel') return
    const after = sentinelLineCount.current
    setAppearAfter((prev) => (shield in prev ? prev : { ...prev, [shield]: after }))
    setShieldOrder((prev) => (prev.includes(shield) ? prev : [...prev, shield]))
  }

  function handleEvent(evt) {
    if (evt.type === 'run_started') {
      setRunId(evt.run_id)
    } else if (evt.type === 'sentinel') {
      setSentinelModel(evt.model)
    } else if (evt.type === 'consent_request') {
      setConsentRequest(evt)
    } else if (evt.type === 'invoke') {
      noteShield(evt.shield)
      setInvocations((prev) => (prev[evt.shield] ? prev : { ...prev, [evt.shield]: evt.reason }))
    } else if (evt.type === 'step') {
      if (evt.shield === 'sentinel') sentinelLineCount.current += 1
      noteShield(evt.shield)
      setShieldSteps((prev) => ({
        ...prev,
        [evt.shield]: [...(prev[evt.shield] || []),
          { text: evt.text, tone: evt.tone, insight: evt.insight }],
      }))
  ...
  function startEvaluation(payload) {
    ...
    setInvocations({})
    setShieldOrder([])
    setSentinelModel(null)
    setAppearAfter({})
    sentinelLineCount.current = 0
    setEvalError(false)
    setRunId(null)
  ...
  function handleChallengeContinue() {
    const timingMs = lastAnswerAt ? Math.round(lastAnswerAt - challengeShownAt.current) : null
    const payload = { ...payloadRef.current, challenge_passed: true, challenge_timing_ms: timingMs }
    console.log('[NeuroSecure] Challenge passed, re-submitting →',
      JSON.stringify(payload.transaction), `answered in ${timingMs}ms`)
    setStep('sending')
  ...
  const challengeElapsedMs = lastAnswerAt ? Math.round(lastAnswerAt - challengeShownAt.current) : null
  const challengePace = challengeElapsedMs === null ? null
    : challengeElapsedMs < 2500 ? 'rushed — possible coaching'
    : challengeElapsedMs > 30000 ? 'unusually slow — possible guidance'
      ...
      <div className="modal">
          ...
          <>
            ...
            {evalError ? (
              ...
              <RiskPanel
                ...
                devMode={devMode}
                holdReason={consentRequest && !consentAnswered ? 'consent' : null}
        ...
        {step === 'challenge' && (
          <>
                    ...
                    {['Yes', 'No'].map((opt) => (
                      <label key={opt} className={challengeAnswers[q.key] === opt ? 'picked' : ''}>
                        <input
                          ...
                          onChange={() => {
                            setChallengeAnswers((a) => ({ ...a, [q.key]: opt }))
                            setLastAnswerAt(performance.now())
                          }}
                        />
                        {opt}
                      </label>
            ...
            {devMode && challengeComplete && challengePace && (
              <p className="cq-pace">
                On-device signal: answered in {(challengeElapsedMs / 1000).toFixed(1)}s — {challengePace}
              </p>
            )}
            <button
              className="btn-continue"
